            ...
            if ("ACTIVE".equalsIgnoreCase(status)) {
                allVouchers = allVouchers.stream()
                    .filter(v -> {
                        ...
                        if (v.getStatus() != VoucherStatus.ACTIVE) return false;
                        // Check if not expired
                        if (v.getExpiryDate() != null && v.getExpiryDate().isBefore(LocalDateTime.now())) return false;
                        // Check if not exceeded usage limit
                        if (v.getMaxUsage() != null) {
                            long usageCount = voucherRepository.countVoucherUsage(v.getId());
                            if (usageCount >= v.getMaxUsage()) return false;
                        }
                        return true;
                    })
                    .toList();
                ...
                allVouchers = allVouchers.stream()
                    .filter(v -> {
                        if (v.getStatus() == VoucherStatus.EXPIRED) return true;
                        if (v.getExpiryDate() != null && v.getExpiryDate().isBefore(LocalDateTime.now())) return true;
                        if (v.getMaxUsage() != null) {
                            long usageCount = voucherRepository.countVoucherUsage(v.getId());
                            if (usageCount >= v.getMaxUsage()) return true;
                        }
                        return false;
                    })
                    .toList();
